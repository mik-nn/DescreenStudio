...
## 2. Архитектура системы (актуально на 2026-09-14; схемы: `knowledge/pipeline.png`, `knowledge/network.png`)

Три дорожки вместо прежних двух блоков (в старой схеме не было A0-замера, ICC-сепарации и возврата тона):

```
A. СИНТЕЗ (P1-симулятор)      GT → ICC-сепарация (пресет→профиль) → AM-растр (LPI 75–190,
                              CMYK/K/дуотон, TVI) → бумага (tint, texture, banding) →
                              скан (поворот, PSF, show-through, шумы) → synth_run4 → обучение run4
                                                     (FM — WONTFIX: муар невозможен)

B. ЗАМЕР БЕЗ GT (A0)          вход → контраст спектра (log+top-hat) → пики+симметрия →
                              (LPI, углы, conf, raster-map) → управляет notch; FiLM — фаза 2 run4

C. ИНФЕРЕНС (Rust)            скан → A0 → адаптивный notch → LF-вычитание → run3 (тайлы) →
                              адаптивный бленд → bg-swap (фон из исходника) → linear-tone → результат

Сеть Fourier-UNet run3: base96/L3, FFC-блоки (spatial + rFFT→1×1→iFFT), residual-выход.
run4-дельты: FFC на L0, enc(2,2)/bneck4, edge-weighted L1 + tone-loss, данные P1.
```
...
| **Этап 4** | **Rust/Tauri Backend** | Интеграция `ort` с DirectML/CoreML, реализация Tiling & Overlap-Add алгоритма. (Скорость: критерий пересмотрен 2026-09-14 — замеренный профиль вместо <2.5 с, см. §9.2.) | 1.5 нед. |
...
2. **Скорость работы (критерий пересмотрен 2026-09-14, см. JOURNAL):** эталонный замеренный профиль на машине разработки — скан А4 (300 DPI, ~3500×2500 px): **≈125 с через DirectML / ≈224 с на CPU** (тайл 512²: ≈2.6 с DML / ≈4.7 с CPU; 48 тайлов). Причина: DirectML EP не содержит FFT-ядер, все DFT-узлы графа исполняются на CPU (fallback); DML ускоряет только свёртки (~×1.8). Целевой порог **<2.5 с** сохраняется как цель оптимизации (бэклог: CUDA12/cuFFT EP, TensorRT auto-FP16, batch-инференс тайлов, больший тайл) и не блокирует Этапы 5–6.
